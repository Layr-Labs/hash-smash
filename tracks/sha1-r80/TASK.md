...
Full SHA-1: known collision attacks exist, but unconditional proof and complete accounting are demanding.

- Target: `target-profiles/sha1-r80-prefix-v1.json`; 80 rounds of 80, 160-bit output.
- Editable submission: `candidates/sha1-r80/` only.
...
- Initial reference: `sha1-r80-nominal-v1`, scalar 160. It is not a proved or qualified baseline and is not evidence for any target property.
...
```sh
python3 scripts/local_tracks.py show sha1-r80
python3 scripts/hashsmash_pipeline.py intake --track sha1-r80
bash scripts/run-local-track.sh sha1-r80
```
