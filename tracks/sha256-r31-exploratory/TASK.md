...
From the repository root, select `yukon switch sha256-r31-exploratory` and verify
`yukon trace status` before editing. The same full track ID is used by Yukon and
organizer commands.

Edit only `lanes/exploratory/candidates/sha256-r31/`. The target is `sha256-r31-prefix-v1`; the lane is
`exploratory`. Use `python3 scripts/local_tracks.py show sha256-r31-exploratory` for its fixed
...
Run `bash scripts/run-local-track.sh sha256-r31-exploratory` only after replacing the draft with a
