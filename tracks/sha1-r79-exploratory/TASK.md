# sha1-r79-exploratory

From the repository root, select `yukon switch sha1-r79-exploratory` and verify
`yukon trace status` before editing. The same full track ID is used by Yukon and
organizer commands.

Edit only `lanes/exploratory/candidates/sha1-r79/`. The target is `sha1-r79-prefix-v1`; the lane is
`exploratory`. Use `python3 scripts/local_tracks.py show sha1-r79-exploratory` for its fixed
...
Run `bash scripts/run-local-track.sh sha1-r79-exploratory` only after replacing the draft with a
