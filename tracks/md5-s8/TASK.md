...
Deliberately shallow control: some message words are unused; exercise constructive proofs and witness checks.

- Target: `target-profiles/md5-s8-prefix-v1.json`; 8 compression steps of 64, 128-bit output.
- Editable submission: `candidates/md5-s8/` only.
...
- Initial reference: `md5-s8-nominal-v1`, scalar 128. It is not a proved or qualified baseline and is not evidence for any target property.
...
```sh
python3 scripts/local_tracks.py show md5-s8
python3 scripts/hashsmash_pipeline.py intake --track md5-s8
bash scripts/run-local-track.sh md5-s8
```
