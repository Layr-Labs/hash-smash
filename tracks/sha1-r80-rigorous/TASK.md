...
From the repository root, select `yukon switch sha1-r80-rigorous` and verify
`yukon trace status` before editing. The same full track ID is used by Yukon and
organizer commands.

Edit only `lanes/rigorous/candidates/sha1-r80/`. The target is `sha1-r80-prefix-v1`; the lane is
`rigorous`. Use `python3 scripts/local_tracks.py show sha1-r80-rigorous` for its fixed
...
Run `bash scripts/run-local-track.sh sha1-r80-rigorous` only after replacing the draft with a
