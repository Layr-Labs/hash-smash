...
From the repository root, select `yukon switch md5-s64-rigorous` and verify
`yukon trace status` before editing. The same full track ID is used by Yukon and
organizer commands.

Edit only `lanes/rigorous/candidates/md5-s64/`. The target is `md5-s64-prefix-v1`; the lane is
`rigorous`. Use `python3 scripts/local_tracks.py show md5-s64-rigorous` for its fixed
